...
for abc in range(0,100):
    ...
    while x:
        try:
                ...
                totaldelay = (linkdelay)+(wait)

                linkdelay_list.append(linkdelay)
                hopcount_list.append(hop_count)
                
                waitdelay_list.append(wait)
                totaldelay_list.append(totaldelay)
                # print(linkdelay_list)
                # print("*"*100)
                

                testingdata = pd.DataFrame({"Hop Count":hopcount_list, "link Delay":linkdelay_list, "Wait Delay":waitdelay_list,"Total Delay":totaldelay_list})
                testingdata.to_csv("C:/Users/Deepa V/Documents/GitHub/Capstone-Project/TESTING/testfiles/DYNAMIC_ADAPTIVE/0_to_15_bitsize_5.csv")
                break
